...
export default function ForgetPassWord() {
    ...
    try {
      conFirmOtpCode(otpCode.value).then((value) => {
        if (value.data) {
          ...
          errMessage.textContent = "Mã xác thực không khớp, vui lòng kiểm tra lại mã otp !!!";
        }
      });
    } catch (err) {
      console.log(err);
  ...
  const submitChangePassword = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/sendEmail/${user.email}/contextConfirmEmail`);
      await axios
        .put("http://localhost:5000/sendEmail/editPassword", {
          ...user,
        })
        .then((value) => {
